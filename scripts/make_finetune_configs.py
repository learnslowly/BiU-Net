...
Usage: python scripts/make_finetune_configs.py
Writes configs/train_{DS}_finetuned_chr{C}.yaml and configs/test_{DS}_finetuned_chr{C}.yaml
"""
import os
...
FAMILIES = {
    "1KGP": dict(
        run_id="1KGP_finetuned",
        source="configs/train_1KGP_finetuned_anchor_chr22.yaml",
        train_on_val=True,
        report_epoch="30",          # fixed schedule, val is in training
        extra=None,
    ),
    "SGDP": dict(
        run_id="SGDP_finetuned",
        source="configs/train_SGDP_finetuned_anchor_chr19.yaml",
        train_on_val=False,
...
for ds, f in FAMILIES.items():
    ...
    for c in CHRS:
        ...
        emit(os.path.join(OUT, f"train_{ds}_finetuned_chr{c}.yaml"), k,
        ...
        emit(os.path.join(OUT, f"test_{ds}_finetuned_chr{c}.yaml"), t,
             HEADER.format(c=c, ds=ds)
