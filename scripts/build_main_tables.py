...
Product (written with a  suffix, originals untouched):
  Results/{DS}_main_table.csv   methods x masking rate, R2 in
      Rare (<=1%) / Low (1-10%) / Common (>10%) / Overall, mean +/- SD over seeds

Usage: python scripts/build_main_tables.py [1KGP|SGDP]
"""
import os
import sys
...
METHODS_BY_DATASET = {
    "1KGP": {
        "BiU-Net": ("1KGP_genomewide", "biunet", SEG, "#1f77b4"),
        "BiU-Net (chr fine-tuned)": ("1KGP_finetuned", "biunet", SEG, "#17becf"),
        "Beagle": ("1KGP_genomewide", "beagle", SEG, "#d62728"),
        "SCDA": ("1KGP_scda_genomewide", "biunet", "seg-1_overlap0", "#2ca02c"),
    },
    # SGDP is read from the standalone runs. Beagle.s cells carry no epoch.
    "SGDP": {
        "BiU-Net": ("SGDP_genomewide", "biunet", SEG, "#1f77b4"),
        "BiU-Net (chr fine-tuned)": ("SGDP_finetuned", "biunet", SEG, "#17becf"),
        "Beagle": ("SGDP_ablation_aligned", "beagle", SEG, "#d62728"),
        "SCDA": ("SGDP_scda_genomewide", "biunet", "seg-1_overlap0", "#2ca02c"),
    },
}
METHODS = METHODS_BY_DATASET[DATASET]
# The checkpoint each method is reported at, matching build_per_chromosome_tables.py so a
...
def main_table(cells):
    ...
    path = os.path.join(OUTDIR, f"genomewide_{DATASET}_main_table.csv")
