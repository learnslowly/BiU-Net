...
Usage: python scripts/plot_beagle_excluded_markers.py
Writes Results/figures/beagle_excluded_markers.pdf and .png plus caption.
"""
import os
import sys
...
SRC = os.path.join(OUTDIR, "beagle_excluded_markers_SGDP.csv")
...
def main():
    ...
    path = os.path.join(FIGDIR, "beagle_excluded_markers.pdf")
    ...
    with open(path.replace(".pdf", "_caption.txt"), "w") as f:
        ...
        f.write("*Source: Results/beagle_excluded_markers_SGDP.csv. "
                "Code: scripts/plot_beagle_excluded_markers.py*\n")
