...
Reads the per-cell CSVs benchmark.py writes to analysis/:
    v3low_ref_g22_phased_{biunet|beagle}_BAT_rand{RS}_1KGP_chr{C}_ALL_seg1024_overlap128_missing{M}.csv
...
_revision2 suffix — never overwrites manuscript originals.
"""
import glob
import os
import re
...
METHODS_BY_DATASET = {
    "1KGP": {
        "biunet": ("v3low_ref_g22", "biunet", "seg1024_overlap128"),
        "beagle": ("v3low_ref_g22", "beagle", "seg1024_overlap128"),
        "scda":   ("scda_fl_g22", "biunet", "seg-1_overlap0"),
    },
    # SGDP is read from the standalone runs. Beagle.s cells carry no epoch.
    "SGDP": {
        "biunet": ("ft_sgdp_full", "biunet", "seg1024_overlap128"),
        "beagle": ("ft_sgdp_aln", "beagle", "seg1024_overlap128"),
        "scda":   ("scda_sgdp_scratch", "biunet", "seg-1_overlap0"),
    },
}
...
def main():
    ...
    p1 = os.path.join(OUTDIR, f"rev2_{DATASET}_per_chromosome_revision2.csv")
    per_chr.to_csv(p1, index=False)
    print(f"wrote {p1}  ({len(per_chr)} rows)")

    gw = genomewide_table(cells)
    p2 = os.path.join(OUTDIR, f"rev2_{DATASET}_genomewide_summary_revision2.csv")
    gw.to_csv(p2, index=False)
    print(f"wrote {p2}  ({len(gw)} rows)")

    h2h = head_to_head(gw)
    p3 = os.path.join(OUTDIR, f"rev2_{DATASET}_headtohead_revision2.csv")
