...
def get_test_masked_file_info(config: ModelConfig, random_states: List[int]) -> List[Tuple[str, str]]:
    ...
    # Train-only segmentation runs (e.g. aligned-ref ALLREF panels) set
    # missing: [] — there are no test/masked inputs for that population tag.
...
def main():
    ...
    # Define random states to process (override via YAML testRandStates if provided)
    random_states = getattr(config, 'testRandStates', None) or [0, 42, 1024]
    ...
    if failed:
        # A silent-success here previously left missing HDF5s behind while the
        # job reported COMPLETED; propagate failure so afterok gates hold.
        print(f"Rank {rank}: {failed} task(s) FAILED; exiting non-zero.")
        sys.exit(1)
