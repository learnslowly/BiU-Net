...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='1KGP', help="Dataset")
    parser.add_argument('--chromosome', type=int, default=6, help="Chromosome number")
    ...
    # Default to HLA suffix for chromosome 6
    if args.chromosome == 6 and args.suffix == '':
        args.suffix = '_HLA'
    return args
...
def process_single_case(args, population, random_state, missing_ratio):
    ...
    """
    # Define input and output paths (suffix appended before _test)
    test_csv_gz = f'./split/{args.dataset}_chr{args.chromosome}_{population}_test{args.suffix}.csv.gz'
    mask_dir = f"./masked/{random_state}/{missing_ratio * 100:.0f}%"
    os.makedirs(mask_dir, exist_ok=True)

    masked_csv_gz = os.path.join(
        mask_dir,
        f"{args.dataset}_chr{args.chromosome}_{population}_missing{missing_ratio * 100:.0f}%{args.suffix}_masked.csv.gz"
    )
