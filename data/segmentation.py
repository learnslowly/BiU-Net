...
def prepare_target_snps(config: ModelConfig, bin_range: str):
    ...
    train_file = f"{config.dataDir}/{config.dataset}/split/{config.dataset}_chr{config.chromosome}_{config.population}_train.csv.gz"
...
def segmentation(config: ModelConfig, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, int]:
    ...
    if segLen == -1:
        ...
        # Vectorized (S, L, 2) build: the tuple-comprehension original
        # materialized S*L Python tuples first (~100 GB of object overhead for
        # a 2548-sample chromosome) and OOM-killed the seg-1 ALLPANEL jobs.
        loci_arr = np.asarray(list(padded_df.index), dtype=np.int64)
        samp_arr = np.asarray(list(padded_df.columns)).astype(np.int64)
        snpsIndex = np.empty((samp_arr.size, loci_arr.size, 2), dtype=np.int64)
        snpsIndex[:, :, 0] = loci_arr[None, :]
        snpsIndex[:, :, 1] = samp_arr[:, None]
...
def de_segmentation(config: ModelConfig, segments_predictions, segments_indices, segments_confidences):
    """
    De-segment and merge predictions using confidence scores.

    Returns:
        restored_df: integer phased class predictions (argmax). Where segments
        overlap, the segment with the highest argmax confidence at that
        (SNP, sample) wins.
    """
