...
def get_dataset_paths(config: ModelConfig, segmentation: bool = False) -> Tuple[List[str], List[str], List[str], List[str]]:
    ...
    dataset_dir = f"{config.dataDir}/{config.dataset}/chunked/"

    # Segmentation runs are single-chromosome by design; training uses the full
    # config.chromosomes list.
    train_chrs = [config.chromosome] if segmentation else (list(config.chromosomes) or [config.chromosome])

    train_csv_gz_files = []
    val_csv_gz_files = []
    for chr_id in train_chrs:
        train_csv_gz_files.extend(glob.glob(os.path.join(
            dataset_dir, f"{config.dataset}_chr{chr_id}_{config.population}_train_chunk*.csv.gz")))
        val_csv_gz_files.extend(glob.glob(os.path.join(
            dataset_dir, f"{config.dataset}_chr{chr_id}_{config.population}_val_chunk*.csv.gz")))
    ...
    if segmentation: # For segmenation
        ...
        # Reference-panel parity: also train on the val-split HDF5s (the Beagle
        # panel is train+val). Val files stay in the val list for monitoring only.
        if getattr(config, 'trainOnVal', False):
            train_hdf5_files = train_hdf5_files + val_hdf5_files
            print(f"trainOnVal: training on {len(train_hdf5_files)} HDF5s "
                  f"(train + {len(val_hdf5_files)} val)")

        # Extra training files (e.g. 1KGP test HDF5s for the fine-tuning runs).
        extra = getattr(config, 'extraTrainFiles', '')
        if extra:
            added = []
            for pat in str(extra).split(','):
                added.extend(sorted(glob.glob(pat.strip())))
            if not added:
                raise FileNotFoundError(f"extraTrainFiles matched nothing: {extra}")
            train_hdf5_files = train_hdf5_files + added
            print(f"extraTrainFiles: +{len(added)} HDF5s merged into training")
...
class HybridFocalLoss(nn.Module):
    ...
    def compute_loss(self, inputs, targets, use_label_smoothing=None):
        ...
        # Move buffer to target device on first call (criterion isn't .to()'d).
        if self.alphas.device != targets.device:
            self.alphas = self.alphas.to(targets.device)
        alpha_t = self.alphas[targets]
    ...
    def forward_masked(self, inputs, targets, valid_mask):
        """Constant-shape variant of forward(), for torch.compile / fused-step paths.

        Args:
            inputs: [N, V] logits, where N = B*S (constant across steps).
            targets: [N] class labels — pad positions may hold padLabel (negative).
            valid_mask: [N] bool — True at positions to include in loss.

        Returns a scalar loss numerically equal to forward(inputs[valid_mask],
        targets[valid_mask]) — verified by semantic test in scripts/.
        """
        # Safe targets for indexing (alpha lookup + cross_entropy reject negative/oob).
        # Masked-out positions get a dummy 0 — their per-elt loss is killed by valid_mask.
        targets_safe = torch.where(valid_mask, targets,
                                   torch.zeros_like(targets))

        # Substitute zeros at invalid positions so fp16 cross entropy cannot
        # overflow. The infinite branch must never be computed, because autograd
        # backpropagates through it.
        inputs_safe = torch.where(valid_mask.unsqueeze(-1), inputs,
                                  torch.zeros_like(inputs))

        # Per-element classification loss (focal weighting on top of CE).
        bce_loss = F.cross_entropy(inputs_safe, targets_safe, reduction='none')  # [N]
        pt = torch.exp(-bce_loss)
        if self.alphas.device != targets.device:
            self.alphas = self.alphas.to(targets.device)
        # alphas covers indices [0..maxClass]; targets_safe is non-negative.
        alpha_t = self.alphas[targets_safe.clamp(max=self.alphas.numel() - 1)]
        focal_loss = alpha_t * (1 - pt) ** self.gamma * bce_loss  # [N]

        # Zero the loss at invalid positions with torch.where rather than a
        # multiply: +inf * 0 is NaN.
        zero_f = torch.zeros_like(focal_loss)
        focal_loss = torch.where(valid_mask, focal_loss, zero_f)
        valid_count = valid_mask.to(focal_loss.dtype).sum().clamp(min=1.0)
        classification_loss = focal_loss.sum() / valid_count

        # Dosage loss (constant-shape: torch.where instead of bool-index scatter).
        # Use inputs_safe so softmax can't NaN at invalid positions either.
        probs = F.softmax(inputs_safe, dim=1)
        zero = torch.zeros_like(targets, dtype=probs.dtype)
        dosage_targets = torch.where((targets_safe == 2) | (targets_safe == 3), 1.0,
                                     torch.where(targets_safe == 4, 2.0, zero))
        predicted_dosage = probs[:, 2] + probs[:, 3] + 2.0 * probs[:, 4]
        dosage_sq = (predicted_dosage - dosage_targets) ** 2
        dosage_sq = torch.where(valid_mask, dosage_sq, torch.zeros_like(dosage_sq))
        dosage_loss = dosage_sq.sum() / valid_count

        return (1 - self.lambda_dosage) * classification_loss + self.lambda_dosage * dosage_loss
...
class HybridWeightedFocalLoss(nn.Module):
    ...
    def compute_loss(self, inputs, targets, use_label_smoothing=None):
        ...
        # Move the penalty matrix to the device of the inputs.
