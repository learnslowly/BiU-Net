...
# The benchmark CSVs key a model by its identifier; the figures carry the name
# the paper uses.
MODEL_LABELS = {'BiUNet': 'BiU-Net'}
...
def plot_metrics_with_segmentation(file_path, output_pdf, align_y=False, model_names=None, metrics_to_plot=None):
    ...
    if model_names is None:
        model_names = {model_id: MODEL_LABELS.get(model_id, str(model_id))
    ...
    if has_different_snp_counts and min_model:
        ...
        snp_legend_elements.append(
            plt.Rectangle((0, 0), 1, 1,
                          color=model_styles[list(model_names.values())[0]]['color'],
    ...
    # Close the PDF file
    pdf.close()
    fig.savefig(output_pdf.replace('.pdf', '.png'), dpi=200, bbox_inches='tight')
...
# Figure 1 is reproduced in the supplement as Figure S4: one render, both names.
for ext in ('pdf', 'png'):
    shutil.copyfile(f'../Figures/Figure_1.{ext}',
                    f'../Figures/Figure_S4_identical_to_Figure_1.{ext}')
