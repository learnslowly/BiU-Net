...
for i, missingness in enumerate(missingness_levels):
    ...
    # The row title names the block beneath it, so it is set a size above the
    # panel titles and sits high in its strip rather than centred in it,
    # which would put it as close to the panels as to the row above.
    title_ax.text(0.5, 0.75, f'Missingness: {missingness_pct}',
...
# The SNP-count bars take the same blue the other per-bin figures use, so a
# reader moving between them reads the same bar as the same thing.
BAR_COLOR = '#1f77b4'
...
# Collect handles and labels for the legend. The lines and the bars measure
# different things on different axes, so each gets its own row rather than
# sharing one where a reader has to notice which entry is a bar.
handles = []
labels = []
snp_handles = []
snp_labels = []
...
for mi, missingness in enumerate(missingness_levels):
    for si, segment_length in enumerate(segment_lengths):
        ...
        for k, metric in enumerate(metrics):
            line, = ax.plot(
                subset['MAF_Bin'], subset[metric], label=metric_label(metric), 
        ...
        bars = ax2.bar(subset['MAF_Bin'], subset['Num_SNPs'], color=BAR_COLOR,
                       alpha=0.3, label='#SNPs')
        # The bars carry a second quantity on a second axis, so the legend has to
        # name them alongside the metrics rather than leave the reader to guess.
        if mi == 0 and si == 0:
            snp_handles.append(bars)
            snp_labels.append('#SNPs')
...
# Add a single legend at the bottom of the subplots. It sits inside the canvas:
# an anchor below zero puts it outside the figure, where the PDF writer clips it.
fig.legend(handles, labels, loc='lower center', ncol=len(metrics), fontsize=12,
           bbox_to_anchor=(0.5, 0.035))
fig.legend(snp_handles, snp_labels, loc='lower center', ncol=1, fontsize=12,
